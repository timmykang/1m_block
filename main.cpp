...
bool packet_check(unsigned char *data) {
	...
	for(int i = 0; i < tcp_data_len - 6; i++) {
		if(!memcmp(tcp_data + i, "Host: ", 6)) {
			for(int j = 0; j < tcp_data_len - i - 8; j++) {
				if(!memcmp(tcp_data + i + j + 6, "\r\n", 2)) {
					string tmp((char *) (tcp_data + i + 6), j);
					if(block_list.find(tmp) != block_list.end())
						return true;
					else
						return false;
				}
...
int main(int argc, char **argv)
{
	...
	if(in.is_open()) {
		...
		while(in) {
			getline(in, tmp);
			block_list.insert(tmp.substr(tmp.find(",") + 1));
		}
	}
	else {
